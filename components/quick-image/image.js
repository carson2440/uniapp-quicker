...
let images = [];//压缩后的图片集合
let max_width = 600;//若宽度大于此尺寸,触发压缩,否则使用原图,可自行修改
...
async function _compress(_image) {
	...
	if (image_info.width < max_width) { //小于max_width不压缩
		return _image;
	}
	...
	return new Promise((resolve, reject) => {
		...
		plus.zip.compressImage({
				...
				quality: 85,//1-100,压缩后质量,越低图片占用空间越小,越模糊
