		 ...
		 @touchstart="touchstart" @touchmove="touchmove" @touchend="touchend" @touchcancel="touchend">
			<view :style="{'transform': translateY, 'transition': transition}">
				<view class="s-pull-down-wrap" :style="{'height':downOffset+'rpx'}">
					<view class="s-pull-loading-icon" :class="{'s-pull-loading-rotate':isDownLoading}" :style="{'transform':downRotate}"></view>
					<view>{{downText}}</view>
				</view>
...
<script>
	export default {
		...
		computed: {
			...
			numTop() {
				return (Array.isArray(this.top) ? this.top : [this.top]).map(num => uni.upx2px(Number(num || 0))).reduce((a, b) =>
					a + b) || 0;
			},
			// bottom数值,单位rpx,需转成px 目的是使上拉布局往上偏移
			numBottom() {
				return (Array.isArray(this.bottom) ? this.bottom : [this.bottom]).map(num => uni.upx2px(Number(num || 0))).reduce((
					a, b) => a + b) || 0;
			},
			numBackTopOffset() {
		...
		methods: {
			...
			triggerPullDown() {
				if (this.pullDown && !this.isDownLoading && !this.isUpLoading) {
					...
					this.pageNo = this.pageStart; // 预先加一页
					this.pullType = 'down';
